...
namespace LauncherCS
{
  public class CreateMapForm : Form
  {
    ...
    private void MapTemplatesListBox_SelectedIndexChanged(object sender, EventArgs e)
    {
      ...
      if (selectedIndex < 0)
        return;
      bool flag = Launcher.IsMultiplayerMapTemplate(this.MapTemplatesListBox.Items[selectedIndex].ToString());
      string name = Launcher.FilterMP(this.MapNameTextBox.Text);
      this.MapNameTextBox.Text = flag ? Launcher.MakeMP(name) : name;
    }
